...
namespace Microsoft.InformationProtectionAndControl
{
    public static class SafeNativeMethods
    {
        ...
        public static IpcApplicationId IpcGetApplicationId()
        {
            IntPtr propertyPtr = IntPtr.Zero;
            try
            {
                ...
                return (IpcApplicationId)Marshal.PtrToStructure(propertyPtr, typeof(IpcApplicationId));
            }
            finally
            {
                UnsafeNativeMethods.IpcFreeMemory(propertyPtr);
        ...
        public static void IpcSetApplicationId(IpcApplicationId id)
        {
            IntPtr propertyPtr = IntPtr.Zero;
            try
            {
                propertyPtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(IpcApplicationId)));
                            ...
                            object credentialType = null)
        {
            ...
            SafeIpcPromptContext ipcPromptContext = CreateIpcPromptContext(suppressUI, offline, hasUserConsent, parentWindow, credentialType);
        ...
        public static NameValueCollection IpcGetSerializedLicenseAppSpecificDataNoEncryption(byte[] license)
        {
            return IpcGetSerializedLicenseAppSpecificData(license, null, LicensePropertyType.AppSpecificDataNoEncryption);
        }
        ...
        public static SafeIpcPromptContext CreateIpcPromptContext(bool suppressUI, bool offline, bool hasUserConsent, Form parentWindow, object credentialType = null)
        {
            return CreateIpcPromptContext(suppressUI, offline, hasUserConsent, IpcWindow.Create(parentWindow).Handle, credentialType);
        }

        public static SafeIpcPromptContext CreateIpcPromptContext(bool suppressUI, bool offline, bool hasUserConsent, IntPtr parentWindow, object credentialType = null)
        {
            ...
            IpcCredential credentials = null;
            if (null != credentialType)
            {
                if (null != symmKey)
                {
                    credentials = new IpcCredential(IpcCredentialType.SymmetricKey, symmKey);
                }
                else if (null != oauth2Key)
                {
                    IpcOAuth2CallbackInfo callbackInfo = new IpcOAuth2CallbackInfo(oauth2Key.MarshallingCallback,
                            oauth2Key.Context);
                    credentials = new IpcCredential(IpcCredentialType.OAuth2, callbackInfo);
                }
                else if (credentialType is IntPtr)
                {
                    credentials = new IpcCredential(IpcCredentialType.X509Certificate, credentialType);
                }
                else
                {
            ...
            if (null != oauth2Key)
            {
                context.flags |= (uint)PromptContextFlag.AcsCredential;
            }
